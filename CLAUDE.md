...
- **macOS Celery pool (`settings.py`):** on macOS (`sys.platform == "darwin"`) `CELERY_WORKER_POOL = "threads"` is set so the worker does **not** use the default `prefork` pool. Reason: forked children **SIGSEGV** when a native library's init touches non-fork-safe Apple frameworks (CoreFoundation/GCD/Kerberos) — reproducibly hit when a worker child runs an Airbyte sync (loads PyAirbyte's `pyarrow` native stack) and then runs a PostgreSQL sync (libpq's Kerberos/GSSAPI init). The `threads` pool avoids `fork()` while keeping concurrency (all tasks are I/O-bound). This is macOS-only; Linux/prod keeps `prefork`. The worker banner should read `concurrency: N (thread)`. Note: the `OBJC_DISABLE_INITIALIZE_FORK_SAFETY=YES` env var can also work but **cannot** be set from Python — the ObjC runtime reads it before Django loads — so the in-code pool override is preferred.

## Environment Variables

> ⚠️ **NEVER read, open, `cat`, or otherwise display `.env`.** It holds live secrets (API keys, encryption key); reading it leaks them into the conversation transcript. To learn which variables exist, use `.env.example` (committed, placeholder values only) or the list below — never the real `.env`. If a task seems to require a real secret's value, stop and ask the user instead of opening `.env`.
