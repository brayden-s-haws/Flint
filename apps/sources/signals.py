...
def delete_periodic_task_on_source_delete(sender: type[SourceSchedule], instance: SourceSchedule, **kwargs: Any) -> None:
    ...
    if instance.periodic_task_id:
        ...
        logger.debug("Deleted periodic task %s on schedule delete for source %s", instance.periodic_task_id, instance.source_id)
...
def cleanup_insights_on_source_delete(sender: type[Source], instance: Source, **kwargs: Any) -> None:
    ...
    insights_qs = Insight.objects.filter(pk__in=insight_ids)
    removed = insights_qs.count()
    insights_qs.delete()
    logger.info("Deleted %s insights on source delete for source %s in account %s", removed, instance.pk, instance.account_id)