...
**Resequenced 2026-08-17:** Phase 6 (cleanup) was pulled ahead of the remaining feature work — build items #15–#22b, now grouped under Phases 7–8b below. Build-item numbers (13–28) are **stable identifiers** referenced across the featuredocs and commit history (e.g. "Phase 6 testing pass (#24)"), so they are **not** renumbered; item numbers are therefore intentionally non-contiguous across phases (Phase 6 = #23–#28 runs before Phase 7 = #15–#20). Read the phase headings top-to-bottom for execution order. Each item still gets its own branch. Phase 6 now covers #23–#25 + #28 (bug bash #26 and AI evals #27 have since been moved out); it still runs before Phase 7 = #15–#20. *(Former Phase 5 → Phase 8, former Phase 5b → Phase 8b; there is no Phase 5 heading after this resequence.)*

23. ~~**Add docstrings to all files** — sweep every module and add module- and function-level docstrings. Task Claude to find all files currently missing them.~~ **COMPLETE (2026-09-03)** — module- and function-level docstrings added across every app (`accounts`, `catalog`, `core`, `users`, `insights`, `sources`) plus the `Flint/` project glue and `demo/` connector. Config/scaffold files (`admin`, `urls`, `apps` without `ready()`, `tests`, `__init__`) intentionally left bare except where novel (signals-importing `apps.py`, the Celery `__init__.py`). Templates deliberately **not** documented — outside the Python-docstring charter; the rendering views describe each partial's purpose instead. `python manage.py check` clean; merged to `main`.
    23a. ~~Comment all apps, comment Flint directory. Should we comment templates?~~ COMPLETE — resolved above; templates skipped by decision.
...
manage.py test`) until it passes. Multi-tenancy boundary tests (account A cannot see account B's data) are required for every tenant-scoped app. Before we start, make sure to review the existing 
    tests and ensure they cover all necessary scenarios. Before we start make sure testing.md is comprehensive of the entire project and apps in the project~~ **COMPLETE (2026-09-12)** — `devdocs/testing.md` fully rewritten as the single source of truth (every app + cross-cutting sweep), then implemented: **186 tests, all passing**; `python manage.py test` green and `manage.py check` clean. Infrastructure: a `TESTING` block in `settings.py` (eager Celery, `TESTING_ENCRYPTION_KEY`, `testserver`, fast hasher), the two-tenant `TenantTestCase` fixture in `apps/core/test_utils.py`, and a new empty `apps/__init__.py` (namespace-package fix so test discovery works). Coverage by app — core 14, users 17, accounts 13, sources 64, catalog 11, insights 65, plus 2 cross-cutting sweeps (`apps/core/tests_cross_product.py`: auth-required redirects + the six `@require_POST` 405s). External I/O mocked at the seam everywhere (LLM `get_service`, `build_connector`, `psycopg2.connect`, `airbyte.ab.get_source`); multi-tenancy A-vs-B boundaries asserted per tenant-scoped view/model. Reviewed by a second agent. **Two carve-outs:** (1) the "Statistics in prompts" test bullets were skipped — that feature isn't implemented (nothing reads `TableStatistics.column_stats` for prompts; no 7-day window); (2) review surfaced a real app-code bug — `AccountSettingsView.get_object` / `SendInviteView.dispatch` raise `AttributeError`→500 (not 403) for an authenticated user with no membership — deferred to the #26 bug bash where it's already tracked.
25. ~~**Address `devdocs/logging.md` in full** — add the `LOGGING` config to `settings.py` and instrument every app per the logging plan (tenant middleware, auth events, sync lifecycle, LLM calls). Never log credentials, tokens, or LLM prompt/response content.~~ **COMPLETE (2026-09-08)** — `LOGGING` dict added to `settings.py` (console at DEBUG behind `require_debug_true`, rotating file at WARNING → `logs/flint.log`, six app loggers + `django.request`, all `propagate: False`). Instrumented every app per `logging.md`: `accounts` (tenant middleware, provisioning signals, invitations), `users` (registration + auth signals), `sources` (encryption, source CRUD, connection test, sync lifecycle, schedule lifecycle, deletion cleanup, connector verify), `catalog` (metadata population + lazy insight trigger), `insights` (four generation tasks, provider selection, cross-source pipeline), and cross-cutting (tenant boundary, degraded HTTP responses via a `@require_POST` refactor + manual 404/400 logging, startup `ENCRYPTION_KEY` presence check). Credentials/tokens/LLM content never logged (one documented exception: cross-source hypothesis *title*, an owner-approved system identifier). Pass also fixed real bugs it surfaced (sync `AttributeError` that failed first syncs, a `text.provider` crash on every source overview, a malformed `logger.exception`). See `logging.md` for per-section notes and items deferred to #26. `python manage.py check` clean throughout; no test suite yet (#24 is next).
28. **Update the README** — revise the README drafted in build order item #9 so it reflects the final feature set. Only create with what we have actually built, do not mention the future 
    ...
    airbyte connector (I have the details for this and can give them to you). Also clean up unnecessary folders/docs from repo - like memory and it's files). Do not include a detailed roadmap 
    section, instead keep it brief and concise and reference that potential roadmap can be found in @post_mvp.md and @potential_features.md 
...
- **Spec-driven Airbyte connect forms (from PyAirbyte `config_spec`)** — supersede the static per-type JSON example shipped with the PyAirbyte adapter (build item #13, Phase 4: the `SourceType.config_example` textarea help/placeholder) with a form generated from the connector's own `config_spec` — render each config property as its own labelled field with the spec's title/description/required flags, instead of one raw JSON `Textarea`. The static example was chosen for #13 because calling `config_spec` triggers the connector's first-use install (slow, needs network) and would stall the HTMX field-swap; a real implementation needs to fetch/cache specs out of the request cycle (e.g. store the spec on `SourceType` at seed/registration time, or a background prefetch). **Natural trigger:** do this once there are enough Airbyte connectors that hand-maintaining `config_example` per type becomes tedious (roughly the SaaS batches — build items #14/#19). See `devdocs/featuredocs/airbyte-adapter.md` (Phase 4, deferred).
...
## testing ✅ COMPLETE (build item #24, 2026-09-12)

The test plan lives in `devdocs/testing.md` — now fully written and implemented (see build item #24 above for the full summary). **186 tests across all six apps + a cross-cutting sweep, all passing**; `python manage.py test` green, `manage.py check` clean. Every tenant-scoped view/model has an A-cannot-see-B boundary test; external I/O (LLM, connectors, psycopg2, PyAirbyte) is mocked at the seam. `testing.md` is the source of truth and tracks the two carve-outs (stats-in-prompts skipped as unimplemented; the membership-less-user 500 deferred to the #26 bug bash).
