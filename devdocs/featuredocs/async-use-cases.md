...
- [ ] **Retry path** — decide between (a) reusing the Generate/Regenerate button from the `failed` state (re-enqueues via the same view, subject to the rate-limit guard), or (b) a dedicated retry that re-enqueues `generate_intra_source_use_cases_task` for the existing placeholder (mirrors `insight_retry`). Note the choice in the section template. No new model either way.
