...
**✅ COMPLETE (2026-08-05):** all steps done — Stripe seeded, `SourceForm` conditional, views branched, `connect_fields` view+URL, `config_example` field, and the HTMX field-swap templates (verified in the UI). The user picks a supported source by name; the native-vs-Airbyte split stays *invisible* — Postgres renders structured `host/port/...` fields, Airbyte-backed types render a single JSON-config `Textarea`, swapped via HTMX. *(Round-trip submit + real Stripe sync = Phase 5.)*
...
- [x] **Step 4 — The dynamic form (HTMX field-swap).** ✅ COMPLETE (2026-08-05) — swap verified in the UI: PostgreSQL renders the five native fields, Stripe renders the JSON config textarea + seeded example, blank shows the prompt. *(Round-trip submit / real Stripe sync is validated in Phase 5.)*
    ...
    **The load-bearing mechanism — `selected_source_type` on every render path.** The partial branches on `selected_source_type`; it must be supplied on all four paths, or full-page renders silently fall through to the Postgres branch (breaking edit-of-Stripe and error re-renders):
    ...
    - [x] **Part 1 — Form (`forms.py`):** in `__init__`, set `self.fields['source_type'].empty_label = 'Select a source type...'` and added HTMX attrs to the `source_type` widget — `hx-get` (via `reverse('sources:connect_fields')`), `hx-target='#connection-fields'`, `hx-trigger='change'`. (`from django.urls import reverse` added.)
    - [x] **Part 2 — Views (`views.py`):** same `get_context_data` on **both** `SourceCreateView` and `SourceUpdateView` — computes `selected_source_type` from `context['form']['source_type'].value()` → `SourceType.objects.filter(pk=raw).first() if raw else None`. Covers the fresh-GET / error-re-render / edit paths in one method.
    - [x] **Part 3 — partial `_connection_fields.html`:** outer `{% if selected_source_type %}` guard (else → prompt). Inside: Airbyte branch → `config` textarea + `config_example` in a copyable `<pre>`; native branch → the five Postgres field blocks. Per-field error lines (`{{ form.<field>.errors }}`) on every field. Existing label/input Tailwind classes reused.
