...
**Status:** Phases 1 & 2 complete — spike + dependency (moved to Python 3.12), and `AirbyteConnector` implemented and verified end-to-end against live Stripe (47 streams mapped, PK + types correct). Phase 3 (registry + `SourceType` routing) is next. See **Phase 1 Spike — Findings** below.
...
- [ ] Add `SourceType.airbyte_connector_name = CharField(max_length=255, blank=True)` — a **non-empty** value marks a SourceType as Airbyte-backed and names the connector (`source-stripe`); native types leave it `''`. (`blank=True`, **no** `null=True` — Django-idiomatic for `CharField`; routing uses a truthy check, so `''` reads as native.) Migration required. *(Alternative: keep it inside the config JSON; rejected because the connector name is metadata about the type, not a per-source secret.)*
...
- [ ] **Add a `build_connector(source_type, credentials) -> BaseConnector` factory to `registry.py`** that returns a *built instance* (not a class), so the native-vs-Airbyte + constructor-arity branching lives in one place: if `source_type.airbyte_connector_name` is set, **lazily** `from .airbyte import AirbyteConnector` (the library is heavy — keep it out of the web process at import time) and return `AirbyteConnector(credentials, source_type.airbyte_connector_name)`; else fall back to the existing native lookup `get_connector(source_type.name)(credentials)`. Type-hint `source_type` via a `TYPE_CHECKING` import of `SourceType` to avoid a runtime import.
- [ ] Update the two call sites — `tasks.py:25-26` and `views.py:169-170` — from the `get_connector(name)` → `connector_class(credentials)` two-liner to a single `build_connector(source.source_type, credentials)` (swap the import too). `sync_source_task`'s body is otherwise unchanged.
