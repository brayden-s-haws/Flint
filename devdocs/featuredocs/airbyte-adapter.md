...
**Status:** Phases 1–3 complete; **Phase 4 nearly done** — Steps 1–3.5 complete (Stripe seeded, conditional `SourceForm`, branched views, `connect_fields` view+URL, `config_example` field); **only Step 4 (templates) remains.** Then Phase 5 (end-to-end Stripe validation via the UI). See **Phase 1 Spike — Findings** below.
...
**In progress (2026-08-04):** Steps 1–3.5 done (Stripe seeded, `SourceForm` conditional, views branched, `connect_fields` view+URL, `config_example` field seeded); **only Step 4 (templates) remains.** The user picks a supported source by name; the native-vs-Airbyte split stays *invisible* — Postgres renders structured `host/port/...` fields, Airbyte-backed types render a single JSON-config `Textarea`, swapped via HTMX.
...
- [x] **Step 2 — `SourceForm` made conditional** (`apps/sources/forms.py`). Postgres fields now `required=False`; added a `config` `Textarea` (`required=False`) for the Airbyte JSON; `clean()` branches on `source_type.airbyte_connector_name` (Airbyte → require + `json.loads`-validate `config`; native → require the five Postgres fields); `source_type` dropdown filtered to `is_demo=False`; the existing `__init__` widget-styling loop already covers the new `config` field. *(Config-help decision: ship a **static per-type JSON example** via a new `SourceType.config_example` field, shown as help text in the partial — see Step 4. The fully **spec-driven** form generated from PyAirbyte `config_spec` is deferred — logged as a `sources` backlog item in `post_mvp.md`, because calling `config_spec` triggers the connector's first-use install and would stall the HTMX swap.)*
- [x] **Step 3 — Views: branch credential handling** (`apps/sources/views.py`, `import json` added).
    - [x] `SourceCreateView.form_valid` **and** `SourceUpdateView.form_valid`: build the credentials dict from `json.loads(form.cleaned_data['config'])` when `source_type.airbyte_connector_name` is set, else the fixed `host/port/dbname/user/password` dict.
    - [x] `SourceUpdateView.get_initial`: Airbyte types pre-fill `config` with `json.dumps(decrypted_creds, indent=2)`; native pre-fills the host/port/... keys.
    - [x] `connect_fields` view + URL (`sources:connect_fields` → `connect-fields/`): reads `?source_type=<id>`, renders the `_connection_fields.html` partial with an unbound `SourceForm()` + `selected_source_type`. `@login_required`.
- [x] **Step 3.5 — `SourceType.config_example` field (static per-type JSON help).** `config_example = TextField(blank=True)` (migration `0009`); Stripe's example JSON seeded via data migration `0010_seed_stripe_config_example` (reversible). Option 1 (static); option 2 (spec-driven) is the deferred `post_mvp.md` item.
- [ ] **Step 4 — The dynamic form (HTMX field-swap).**
    **Design decision — initial-load UX = empty prompt (Option A).** On a fresh "Add source" load, the `source_type` dropdown starts on a blank "Select a source type…" option and the connection-fields area shows a placeholder; fields appear only after a type is chosen. This means the `<select>` and the fields can never disagree, and it sidesteps "which fields by default?". *(Rejected Option B — pre-select the first type — as more coupling + a worse first impression, e.g. showing a JSON textarea before any choice.)*

    **The load-bearing mechanism — `selected_source_type` on every render path.** The partial branches on `selected_source_type`; it must be supplied on all four paths, or full-page renders silently fall through to the Postgres branch (breaking edit-of-Stripe and error re-renders):
    | Path | Source of `selected_source_type` |
    |---|---|
    | HTMX swap (select changed) | `connect_fields` view, from `?source_type=<id>` |
    | Fresh create GET | `get_context_data` → `None` → prompt |
    | Validation-error re-render | `get_context_data` → submitted PK |
    | Edit (UpdateView) | `get_context_data` → the source's type PK |

    - [ ] **Part 1 — Form (`forms.py`):** in `__init__`, set `self.fields['source_type'].empty_label = 'Select a source type…'` and add HTMX attrs to the `source_type` widget — `hx-get` (via `reverse('sources:connect_fields')`, since `{% url %}` isn't available in Python), `hx-target='#connection-fields'`, `hx-trigger='change'`.
    - [ ] **Part 2 — Views (`views.py`):** add the *same* `get_context_data` to **both** `SourceCreateView` and `SourceUpdateView` — compute `selected_source_type` from `context['form']['source_type'].value()` (submitted PK, instance PK, or `None`) → `SourceType.objects.filter(pk=raw).first() if raw else None`. Covers the fresh-GET / error-re-render / edit paths in one method.
    - [ ] **Part 3 — New partial `_connection_fields.html`:** outer `{% if selected_source_type %}` guard (else → the "Select a source type…" prompt). Inside: `{% if selected_source_type.airbyte_connector_name %}` → the `config` textarea **plus `selected_source_type.config_example` as copyable help** (a `<pre>` block, not a `placeholder`); `{% else %}` → the five Postgres field blocks. **Add a per-field error line** (`{{ form.<field>.errors }}`) to every field — `clean()` now attaches errors to specific fields, so they must render inline, not only in the bottom non-field block. Reuse the existing label/input Tailwind classes.
    - [ ] **Part 4 — `source_form.html`:** replace the hardcoded "Connection Details" heading + five field blocks with `<div id="connection-fields">{% include 'sources/_connection_fields.html' %}</div>`. The `source_type` `<select>` already carries the `hx-*` attrs from Part 1; the include inherits `form` + `selected_source_type` from the page context. Leave `name`, the `source_type` block, and the submit button as-is.
    - [ ] **Part 5 (robustness) — `connect_fields` empty guard:** swap `get_object_or_404(...)` for `raw = request.GET.get('source_type'); source_type = SourceType.objects.filter(pk=raw).first() if raw else None` so re-selecting the blank option (`?source_type=`) renders the prompt instead of 404-ing an error fragment into the form.
