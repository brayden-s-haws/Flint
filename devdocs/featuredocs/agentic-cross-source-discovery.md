...
**Status:** Phase 1 in progress — models + prompt module + service layer done; pipeline started (`_flatten_and_rank_relationships` done, `run_discovery_for_pair` + storage next)
...
- [ ] `apps/insights/cross_source_pipeline.py` — orchestration function `run_discovery_for_pair(source_a, source_b, run) -> int` that calls Step 3 → Step 4 → (Phase 1: no Step 5 dedup) → Step 6 → Step 7 and returns insights-created count. Plain Python; LLM calls via `get_service(...)`.
  ...
  - [ ] `run_discovery_for_pair(...)` orchestration body — Step 3 → flatten/rank → cap top ~3 → Step 4 fan-out + pool → sort pooled by `specificity_score`, top 5 → Step 6 → store. Per-item `try/except` around Step 4 and Step 6+store so one bad item doesn't sink the run.
  - [ ] `_store_cross_source_insight(insight_data, source_a, source_b) -> Insight` — non-destructive create (one `Insight`, two `InsightTarget` rows, `status='pending_review'`), wrapped in `transaction.atomic()`.
