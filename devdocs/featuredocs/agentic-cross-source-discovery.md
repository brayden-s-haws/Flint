...
- [x] `run_cross_source_discovery` — `POST /insights/discovery/run/` — accepts two `source_id`s (the explicitly chosen pair), validates both belong to `request.account` and are synced, enqueues the pipeline Celery task. Rate-limited to once / 24h (mirror the `generate_intra_use_case_suggestions` guard). **Appends** new insights (non-destructive — never deletes prior ones). Returns the results-section partial. *(Done in `apps/insights/views.py`: rejects self-pairing and unsynced sources (400), per-pair 24h rate-limit via a double `insighttarget` filter, enqueues `run_cross_source_discovery_task.delay`, returns `_cross_source_discovery_results.html` via `build_cross_source_results_context(..., since=now)`. URL pattern still TODO.)*
- [x] `cross_source_discovery_status` — `GET /insights/discovery/status/` — HTMX poll endpoint that renders the results section (spinner while running, then the newest-first list with new insights at the top), matching the async-on-first-view pattern in `devdocs/featuredocs/async-table-descriptions.md`. Reuse `build_cross_source_results_context`, which already `.exclude(status='dismissed')`s (see the "Dismiss is permanent removal" decision) — so dismissed cards don't resurface on a poll. *(Done: reads `?since`, `datetime.fromisoformat` with an empty/missing guard, delegates to the helper. No `insight_id` — polls the whole run.)*
- [x] `accept_agent_insight` — `POST /insights/<insight_id>/accept/` — flips `pending_review` → `active`, swaps the card row (HTMX). *(Done: scoped to `account` + `insight_type='cross_source_use_case'`, `pending_review` guard → 400, re-renders `_agent_insight_card.html` so the accepted card stays visible.)*
- [x] `dismiss_agent_insight` — `POST /insights/<insight_id>/dismiss/` — flips → `dismissed`, swaps the card row. *(Done: same guards as accept; returns an empty `HttpResponse('')` so HTMX swaps the card **away** — per the "Dismiss is permanent removal" decision, not a card re-render.)*
- [x] All views `@login_required` and scope every queryset by `account=request.account`.
- [x] Add the URL patterns to `apps/insights/urls.py` under `app_name = 'insights'`. *(Done: `discovery/`, `discovery/run/`, `discovery/status/`, `<int:insight_id>/accept/`, `<int:insight_id>/dismiss/`; `manage.py check` clean.)*
